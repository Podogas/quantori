      ...
      <div key={task.id} className={`${blockName}__task`}>
        ...
        <div className={`${blockName}__task-info-wrapper`}>
          ...
          <div className={`${blockName}__task-caption-wrapper`}>
            <span
              className={`${blockName}__task-tag ${blockName}__task-tag--${
                task.isCompleted ? "completed" : task.tag
              }`}
            >
              {task.tag}
            </span>
        ...
        {task.isCompleted ? null : (
          <div className={`${blockName}__task-buttons-wrapper`}>
            <button
              className={`${blockName}__task-button ${blockName}__task-edit-button`}
              type="button"
              ref={editBtnRef}
              onClick={() => onEditClick(task)}
            ></button>
            <button
              className={`${blockName}__task-button ${blockName}__task-delete-button`}
              type="button"
              ref={deleteBtnRef}
