...
const Tags = ({
    ...
    const selectTag = (n: number) => {
        ...
        if(element?.classList.contains('info-tag--selected')){
          ...
          tagsRef.current.forEach(e => e.current?.classList.remove('info-tag--selected'))
    ...
    <div className={`${blockName}__info-tags-wrapper info-tags-wrapper`}>
        <span className={`${blockName}__info-tag info-tag info-tag--health ${healthTagSelected ? 'info-tag--selected' : ''}`} ref={tagsRef.current[0] } onClick={() => {selectTag(0)}}>health</span>
        <span className={`${blockName}__info-tag info-tag info-tag--work ${workTagSelected ? 'info-tag--selected' : ''} `} ref={tagsRef.current[1]}  onClick={() => {selectTag(1)}}>work</span>
        <span className={`${blockName}__info-tag info-tag info-tag--home ${homeTagSelected ? 'info-tag--selected' : ''} `} ref={tagsRef.current[2]}  onClick={() => {selectTag(2)}}>home</span>
        <span className={`${blockName}__info-tag info-tag info-tag--other ${otherTagSelected ? 'info-tag--selected' : ''} `} ref={tagsRef.current[3]}  onClick={() => {selectTag(3)}}>other</span>
    </div>
  )
}
