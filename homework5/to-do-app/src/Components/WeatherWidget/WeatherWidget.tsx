...
const WeatherWidget = ({ blockName }: { blockName: string }) => {
  const [weather, setWeather] = useState<WeatherDataType | undefined>(
    undefined
  );
  ...
  if (weather) {
    return (
      <div className={`${blockName}__weather-widget`}>
        <img
          className={`${blockName}__weather-widget-icon`}
          src={weather.icon.url}
          alt={weather.icon.description}
        />
        <span className={`${blockName}__weather-widget-tempreture`}>
          {weather.temp}
        </span>
        <span className={`${blockName}__weather-widget-location`}>
