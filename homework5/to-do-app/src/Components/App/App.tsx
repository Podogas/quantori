...
const App = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchTasks()).then((res) => shouldShowModal(res.payload));
  ...
  const shouldShowModal = (tasks: TaskType[]) => {
    ...
    if (lastVisit && lastVisit !== formatDate(new Date())) {
      const tasksForToday = tasks.filter(
        (task) => task.date === lastVisit && task.isCompleted === false
      );
    ...
    if (!lastVisit) {
      const tasksForToday = tasks.filter(
        (task) =>
          task.date === formatDate(new Date()) && task.isCompleted === false
      );
      if (tasksForToday.length !== 0) {
        setPopupContent(tasksForToday);
        setPopupType("modal");
