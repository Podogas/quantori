...
const fetchTasks = createAsyncThunk("tasks/get", async () => {
...
const taskSlice = createSlice({
  ...
  reducers: {
    ...
    editTask: (state, action: PayloadAction<TaskType>) => {
      state.uncompleted = state.uncompleted.map((t) =>
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchTasks.fulfilled, (state, action) => {
      const [completed, uncompleted] = action.payload.reduce(
        (acc: Array<TaskType[]>, task: TaskType) => {
